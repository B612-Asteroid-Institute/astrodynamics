...
public class DistanceFromEarthTargeterTest {
  ...
  private double finalStateVectorDistanceFromEarth(
      String ephemeris, ReferenceFrame referenceFrame) {
    StkEphemerisFile file =
        StkEphemerisFile.readFrom(
            new BufferedReader(
                new InputStreamReader(new ByteArrayInputStream(ephemeris.getBytes()))));
  ...
  public void testInitialManeuver() throws AdamPropagationException, IOException {
    ...
    JulianDate startEpoch =
        new JulianDate(2456437.50002315, TimeStandard.getCoordinatedUniversalTime());
    JulianDate stopEpoch =
        new JulianDate(2458263.62933198, TimeStandard.getCoordinatedUniversalTime());
    Duration duration =
        stopEpoch
            .subtractDays(DAYS_BEFORE_IMPACT_TO_SWITCH_TO_TARGETING_PERIGEE)
            .subtract(startEpoch);
    ...
    Motion1<Cartesian> initialState =
        new Motion1<Cartesian>(
            ...
            new Cartesian(
                -11.2528789273597756e+03, -22.3258231726462242e+03, -9.7271222877710155e+03));
    ...
    targeter.initialize(
        initialState, sunIcrf, startEpoch, initialManeuver, duration, targetingParams, config);

    // No maneuver should be required because the initial maneuver should have been enough to push
    // it away.
  ...
  public void testNominalOnly() throws AdamPropagationException, IOException {
    ...
    JulianDate startEpoch =
        new JulianDate(
            2456437.50002315,
            TimeStandard.getCoordinatedUniversalTime()); // 25 May 2013 00:00:02.00016 UTCG
    JulianDate stopEpoch =
        new JulianDate(
            2458263.62933198,
            TimeStandard.getCoordinatedUniversalTime()); // 25 May 2018 03:06:14.28307 UTCG
    Duration duration =
        stopEpoch
            .subtractDays(DAYS_BEFORE_IMPACT_TO_SWITCH_TO_TARGETING_PERIGEE)
            .subtract(startEpoch);
    ...
    targeter.initialize(
        initialState, sunIcrf, startEpoch, new Cartesian(), duration, targetingParams, config);

    Assert.assertTrue(targeter.runNominalPropagationToCheckRequiresManeuver(targetingParams));
    // Running propation isn't necessary - checking for a maneuver required already runs the nominal
    // propagation.

    // Final state vector will be too close to earth since targeting didn't run.
    double minDistanceFromEarth =
        targetingParams.getTargetDistanceFromEarth() * 1000.0
            - targetingParams.getTolerance() * 1000.0;
    Assert.assertTrue(
        finalStateVectorDistanceFromEarth(targeter.getEphemeris(), sunIcrf) < minDistanceFromEarth);
  }
  ...
  public void testDesktopScenario1() throws AdamPropagationException, IOException {
    ...
    // Check that the final state vector of the ephemeris is indeed within tolerance of the target
    // distance from earth.
    Assert.assertEquals(
        targetingParams.getTargetDistanceFromEarth() * 1000.0,
        finalStateVectorDistanceFromEarth(targeter.getEphemeris(), sunIcrf),
        targetingParams.getTolerance() * 1000.0);
  }
  ...
  public void testDesktopScenario2() throws AdamPropagationException, IOException {
    ...
    JulianDate startEpoch =
        new JulianDate(
            2457175.00002315,
            TimeStandard.getCoordinatedUniversalTime()); // 1 Jun 2015 12:00:02.00000
    JulianDate stopEpoch =
        new JulianDate(
            2460827.49900783,
            TimeStandard.getCoordinatedUniversalTime()); // 31 May 2025 23:58:34.27651 UTCG
    Duration duration =
        stopEpoch
            .subtractDays(DAYS_BEFORE_IMPACT_TO_SWITCH_TO_TARGETING_PERIGEE)
            .subtract(startEpoch);
    ...
    Motion1<Cartesian> initialState =
        new Motion1<Cartesian>(
            // Location, values in meters.
            new Cartesian(
                -1.3162148431055502e+11, -1.0215322647665953e+11, -5.8415540509782318e+10),
            // Velocity, values in meters / second.
  ...
  public void testDesktopScenario3() throws AdamPropagationException, IOException {
    ...
    JulianDate startEpoch =
        new JulianDate(
            2457327.75001157,
            TimeStandard.getCoordinatedUniversalTime()); // 1 Nov 2015 06:00:00.99965 UTCG
    JulianDate stopEpoch =
        new JulianDate(
            2462806.5,
            TimeStandard.getCoordinatedUniversalTime()); // 1 Nov 2030 00:00:00.00000 UTCG
    Duration duration =
        stopEpoch
            .subtractDays(DAYS_BEFORE_IMPACT_TO_SWITCH_TO_TARGETING_PERIGEE)
            .subtract(startEpoch);
    ...
    Motion1<Cartesian> initialState =
        new Motion1<Cartesian>(
            // Location, values in meters.
            new Cartesian(
                -6.1734205715280347e+11, -1.0066137279860652e+11, -9.7607883465475815e+10),
            // Velocity, values in meters / second.
  ...
  public void testDesktopScenario4() throws AdamPropagationException, IOException {
    ...
    //
    // The initial conditions that match the desktop don't actually result in a collision. Confirm
    // that
    // this results in a targeter that does not require a maneuver. Then propagate backwards from
    // the
    ...
    JulianDate startEpoch =
        new JulianDate(
            2441257.2503125,
            TimeStandard.getCoordinatedUniversalTime()); // 1 Nov 1971 18:00:27.00000 UTCG
    JulianDate stopEpoch =
        new JulianDate(
            2459519.75,
            TimeStandard.getCoordinatedUniversalTime()); // 1 Nov 2021 06:00:00.00000 UTCG
    Duration duration =
        stopEpoch
            .subtractDays(DAYS_BEFORE_IMPACT_TO_SWITCH_TO_TARGETING_PERIGEE)
            .subtract(startEpoch);
    ...
    // in components - in fact, they result in a flyby that is outside the target distance from
    // Earth.
    // This should be detectable using requiresManeuver.
    //
    // Note: this is because of a very close intermediate flyby that amplifies the difference
    // between
    ...
    // The ephemeris retrieved from the nominal run should show that the object ends up far from
    // earth.
    double minTargetDistance =
        targetingParams.getTargetDistanceFromEarth() * 1000.0
            - targetingParams.getTolerance() * 1000.0;
    Assert.assertTrue(
        minTargetDistance <= finalStateVectorDistanceFromEarth(targeter.getEphemeris(), sunIcrf));

    // These are the coordinates propagated backwards from the point of collision as output from the
    // desktop.
    // They do result in an impact and therefore require a maneuver.
    Motion1<Cartesian> initialState2 =
        new Motion1<Cartesian>(
            // Location, values in meters.
    ...
    targeter.initialize(
        initialState2, sunIcrf, startEpoch, new Cartesian(), duration, targetingParams, config);

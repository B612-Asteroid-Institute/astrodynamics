...
public class OdmFormatterTest {
  ...
  /**
   * Returns OPM from figure 3-1 in the ODM standard but USING 'TT' as the TIME_SYSTEM instead of
   * 'UTC'
   */
  ...
  public void testParseKeplerianAndManeuversOpmHappy() throws Exception {
    ...
    OrbitParameterMessage parsed =
        OdmFormatter.parseOpmString(OdmScenarioBuilder.getOpmWithKeplerianAndManeuvers());
    Assert.assertEquals(parsed, expected);
  }
  ...
  public void testCovarianceOpmHappy() throws Exception {
    ...
    OrbitParameterMessage parsed =
        OdmFormatter.parseOpmString(OdmScenarioBuilder.getOpmWithCovariance(type));
    Assert.assertEquals(parsed, expected);
  }
  ...
  public void testBasicOemHappy() throws Exception {
    ...
    OrbitEphemerisMessage parsed =
        OdmFormatter.parseOemString(OdmScenarioBuilder.getOemWithAccelerations());
    Assert.assertEquals(parsed, expected);
  }
  ...
  public void testOemWithCovariance() throws Exception {
    ...
    OrbitEphemerisMessage parsed =
        OdmFormatter.parseOemString(OdmScenarioBuilder.getOemWithCovariance());
    Assert.assertEquals(parsed, expected);
  }
  ...
  public void testBasicOorbOemHappy() {
    ...
    OrbitEphemerisMessage oemFromOorb =
        OdmFormatter.parseOorbEphemerisString(oorbEphemerisString, objectName, objectId, false);
    ...
    // 2019-01-02T00:01:09 2.2439016200494885E8 2101124.491121584 -1.3365823487439504E-7
    // -0.22770539879383658 24.31785994349861 -3.052498245421107E-12
  ...
  public void testOorbOemWithFrameConversionHappy() {
    ...
    String oorbEphemerisString =
        String.join(
            ...
            "smoke CAR  0.149975833233652E+01  0.280890681682720E-01 -0.345024787678361E-14 -0.263010213738389E-03  0.140428922497122E-01 -0.329957019328539E-14  0.200000000000000E+02   0.58486000800741E+05 1 6 -0.100000000000000E+01 OPENORB");
    OrbitEphemerisMessage oemFromOorb =
        OdmFormatter.parseOorbEphemerisString(oorbEphemerisString, objectName, objectId, true);
    assertEquals(1, oemFromOorb.getBlocks().size());
    OemDataBlock block = oemFromOorb.getBlocks().get(0);
    OemMetadata metadata = block.getMetadata();
    String expectedStart = "2019-01-02T00:01:09.184022373";

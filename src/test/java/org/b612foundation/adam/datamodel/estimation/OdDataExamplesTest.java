...
public class OdDataExamplesTest {
  ...
  public void configuringInputDataExample() throws JsonProcessingException {
    ...
    // Example of configuring a propagator that would be referenced by UUID in the force models of the OD config
    ...
    // which should be swappable transparently hopefully/hypothetically.  One is for the Thor IOD capabilities
    // the other is for OpenOrb Ranging IOD capabilities (these aren't actual settings to be used on above data
    ...
    var oorbIodConfig = OrbitDeterminationConfiguration.builder()
        .executor("OpenOrb")
    ...
    // Example of two separate IOD configurations for two different IOD systems
    // which should be swappable transparently hopefully/hypothetically.  One is for the OpenOrb OD capabilities
    // the other is for Orekit OD capabilities (these aren't actual settings to be used on above data
    // but representative ones to show the variety of system-specific elements that can be configured

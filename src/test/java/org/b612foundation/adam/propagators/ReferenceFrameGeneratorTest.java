...
public class ReferenceFrameGeneratorTest {
  ...
  @Test
  public void testGetCentralBody() {
    StkLicense.Activate();
  ...
  @Test
  public void testGetICRF() {
    StkLicense.Activate();
    ReferenceFrameGenerator rfg = new ReferenceFrameGenerator();

    ReferenceFrame earthICRF = rfg.getReferenceFrame(OdmCommonMetadata.ReferenceFrame.ICRF,
        OdmCommonMetadata.CenterName.EARTH);
    Assert.assertEquals(EARTH.getCenterOfMassPoint(), earthICRF.getOrigin());

    ReferenceFrame sunICRF = rfg.getReferenceFrame(OdmCommonMetadata.ReferenceFrame.ICRF,
        OdmCommonMetadata.CenterName.SUN);
    Assert.assertEquals(SUN.getCenterOfMassPoint(), sunICRF.getOrigin());
  }

  @Test
  public void testEMEME2000() {
    StkLicense.Activate();
    ReferenceFrameGenerator rfg = new ReferenceFrameGenerator();

    ReferenceFrame earthEMEME2000 = rfg.getReferenceFrame(OdmCommonMetadata.ReferenceFrame.EMEME2000,
        OdmCommonMetadata.CenterName.EARTH);
    Assert.assertEquals(EARTH.getCenterOfMassPoint(), earthEMEME2000.getOrigin());

    ReferenceFrame sunEMEME2000 = rfg.getReferenceFrame(OdmCommonMetadata.ReferenceFrame.EMEME2000,
        OdmCommonMetadata.CenterName.SUN);
    Assert.assertEquals(SUN.getCenterOfMassPoint(), sunEMEME2000.getOrigin());
  }

  private Motion1<Cartesian> transformMotion(Motion1<Cartesian> in, ReferenceFrame from, ReferenceFrame to,
      JulianDate at) {
    ReferenceFrameEvaluator evaluator = GeometryTransformer.getReferenceFrameTransformation(from, to);
    int order = 1;
    KinematicTransformation transformer = evaluator.evaluate(at, order);
    return transformer.transform(in, order);
  }

  private Cartesian transformPoint(Cartesian in, ReferenceFrame from, ReferenceFrame to, JulianDate at) {
    ReferenceFrameEvaluator evaluator = GeometryTransformer.getReferenceFrameTransformation(from, to);
    int order = 1;
    KinematicTransformation transformer = evaluator.evaluate(at, order);
  ...
  @Test
  public void testAllMagnitudesEqual() {
    StkLicense.Activate();
    ReferenceFrameGenerator rfg = new ReferenceFrameGenerator();

    List<OdmCommonMetadata.ReferenceFrame> supportedReferenceFrames = Arrays
        .asList(OdmCommonMetadata.ReferenceFrame.EMEME2000, OdmCommonMetadata.ReferenceFrame.ICRF);
    List<OdmCommonMetadata.CenterName> supportedCenters = Arrays.asList(OdmCommonMetadata.CenterName.SUN,
        OdmCommonMetadata.CenterName.EARTH);

    // Check that for a given center, a point has the same magnitude in all supported reference frames.
    for (OdmCommonMetadata.CenterName centerName : supportedCenters) {
      ...
      for (OdmCommonMetadata.ReferenceFrame referenceFrameName : supportedReferenceFrames) {
        ...
        Assert.assertEquals("For center " + centerName.name() + ", reference frame " + referenceFrameName.name()
            + ", magnitude should remain constant.", point.getMagnitude(), transformed.getMagnitude(), 1e-10);

        // Compared for the same central body, distance of a non-moving point from the center should be time-invariant
        // and reference-frame-invariant.
        transformed = transformPoint(point, defaultFrame, referenceFrame, whenever.addHours(1));
        Assert.assertEquals(
            "For center " + centerName.name() + ", reference frame " + referenceFrameName.name()
                + ", magnitude constancy should be time-invariant.",
            point.getMagnitude(), transformed.getMagnitude(), 1e-10);
      }
    }
  }
  
  @Test
  public void testMatchDesktopTransformation() {
    StkLicense.Activate();
    ...
    ReferenceFrame j2000 = new ReferenceFrame(CentralBodiesFacet.getFromContext().getSun().getCenterOfMassPoint(),
        CentralBodiesFacet.getFromContext().getEarth().getJ2000Frame().getAxes());
    ReferenceFrame icrf = new ReferenceFrame(CentralBodiesFacet.getFromContext().getSun().getCenterOfMassPoint(),
        CentralBodiesFacet.getFromContext().getEarth().getInternationalCelestialReferenceFrame().getAxes());
    ...
    Cartesian expectedPos = new Cartesian(999999.9049605389700, 999999.9880806692545, 1000000.1069587816573);
    ...
    System.out.println("Actual: " + transformed.getValue() + "; " + transformed.getFirstDerivative());
    for (int i = 0; i < 3; i++) {
      Assert.assertEquals("Checking correct position transformation, component " + i, expectedPos.get(i),
          transformed.getValue().get(i), positionEps);
      Assert.assertEquals("Checking correct velocity transformation, component " + i, expectedVel.get(i),
          transformed.getFirstDerivative().get(i), velocityEps);
    }
  }

  @Test
  public void testHeliocentricEMEME2000ToIcrf() {
    StkLicense.Activate();
    ...
    Cartesian icrfPosition = new Cartesian(999999.9380994121339, 519704.8311468433894, 1315259.4438022811119);
    Cartesian icrfVelocity = new Cartesian(999.9997580464747, 121.9276229688568, 2232.7414832199990);

    JulianDate whenever = TimeConstants.J2000; //.addDays(12345); // Arbitrary, shouldn't matter.
    Motion1<Cartesian> transformed = transformMotion(memeMotion, sunEMEME2000, sunICRF, whenever);

    // Correct within a large margin. Not sure why such a high threshold should be necessary, but the transformation
    ...
    System.out.println("Actual: " + transformed.getValue() + "; " + transformed.getFirstDerivative());
    for (int i = 0; i < 3; i++) {
      Assert.assertEquals("Checking correct position transformation, component " + i, icrfPosition.get(i),
          transformed.getValue().get(i), positionEps);
      Assert.assertEquals("Checking correct velocity transformation, component " + i, icrfVelocity.get(i),
          transformed.getFirstDerivative().get(i), velocityEps);
    }
  }
  ...
  public void testSunInertialFrameIsICRFNotJ2000() {
    ...
    ReferenceFrame j2000 = new ReferenceFrame(SUN.getCenterOfMassPoint(), SUN.getJ2000Frame().getAxes());
    ReferenceFrame sunICRF = new ReferenceFrame(SUN.getCenterOfMassPoint(),
        EARTH.getInternationalCelestialReferenceFrame().getAxes());

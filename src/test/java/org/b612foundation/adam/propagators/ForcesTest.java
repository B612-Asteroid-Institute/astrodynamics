...
public final class ForcesTest {
  ...
  protected static final OrbitParameterMessage INITIAL_OPM =
      new OrbitParameterMessage()
          ...
          .setHeader(
              new OdmCommonHeader()
                  .setCreation_date("2016-10-22")
                  .setOriginator("B612-jcarrico"))
          .setMetadata(
  ...
  public void testDatesOutOfRange() throws Exception {
    ...
    try {
      runOneTest(INITIAL_OPM, tooEarly, tooEarly,
          // The rest of these parameters don't matter.
          new Cartesian(7.3978163610693619e10, -1.2182276005571476e11, -5.2811158832497582e10),
          new Cartesian(31.7100034398931780e3, 29.9657246374751020e3, 0.6754531613947713e3), 4.5, // meters
    ...
    try {
      runOneTest(INITIAL_OPM, tooLate, tooLate,
          // The rest of these parameters don't matter.
          new Cartesian(7.3978163610693619e10, -1.2182276005571476e11, -5.2811158832497582e10),
          new Cartesian(31.7100034398931780e3, 29.9657246374751020e3, 0.6754531613947713e3), 4.5, // meters
          0.01); // limit to 1 cm/s; actual < 5 mm
  ...
  public void testAtPerigee() throws Exception {
    ...
    runOneTest(
        INITIAL_OPM,
        new JulianDate(
            ZonedDateTime.parse("2009-07-21T21:55:08.813Z"), TimeStandard.getCoordinatedUniversalTime()),
  ...
  public void testTenYearsAfterSwingBy() throws Exception {
    runOneTest(
        INITIAL_OPM,
        new JulianDate(
            ZonedDateTime.parse("2019-07-22T09:55:05.813Z"), TimeStandard.getCoordinatedUniversalTime()),
  ...
  public void testEmeme() {
    // This test uses all the same numbers as testAtPerigee, just transformed to sun-centered Earth mean ecliptic mean
    // equinox.
    ...
    ReferenceFrameEvaluator eval = GeometryTransformer.getReferenceFrameTransformation(
        rfg.getReferenceFrame(OdmCommonMetadata.ReferenceFrame.ICRF, OdmCommonMetadata.CenterName.SUN),
        rfg.getReferenceFrame(OdmCommonMetadata.ReferenceFrame.EMEME2000, OdmCommonMetadata.CenterName.SUN));

    JulianDate startDate = new JulianDate(ZonedDateTime.parse(opm.getState_vector().getEpoch()),
        TimeStandard.getCoordinatedUniversalTime());
    JulianDate endDate = new JulianDate(ZonedDateTime.parse("2009-07-21T21:55:08.813Z"),
        TimeStandard.getCoordinatedUniversalTime());
    JulianDate endDateFromNumber = new JulianDate(2455034.41329645, TimeStandard.getCoordinatedUniversalTime());

    StateVector state = opm.getState_vector();
    Motion1<Cartesian> icrfInitialState = new Motion1<Cartesian>(
    ...
    Motion1<Cartesian> ememeInitialState = eval.evaluate(startDate, 2).transform(icrfInitialState, 2);
    opm.setState_vector(new StateVector()
    ...
    Motion1<Cartesian> icrfFinalState = new Motion1<Cartesian>(
        new Cartesian(7.3978163610693619e10, -1.2182276005571476e11, -5.2811158832497582e10),
        new Cartesian(31.7100034398931780e3, 29.9657246374751020e3, 0.6754531613947713e3));
  ...
  public void testKeplerianElements() {
    // This test is exactly like testTenYearsAfterSwingBy except that we use Keplerian elements instead of cartesian.
    OrbitParameterMessage opm = INITIAL_OPM.deepCopy();
    ...
    KeplerianElements keplerian = new KeplerianElements(initialPosition, initialVelocity, sunGravity);
    // STK units are m^3/s^2 and m, OPM units are km^3/s^2 and km.
    // STK units are radians, OPM units are degrees.
    opm.setKeplerian(new org.b612foundation.adam.opm.KeplerianElements()
    ...
    // 30 km; so large because the conversion between keplerian and cartesian coordinates is not exact.
    double positionEps = 30000;
    // 1 cm/s
    double velocityEps = 0.01;
    runOneTest(
        opm,
        new JulianDate(ZonedDateTime.parse("2019-07-22T09:55:05.813Z"), TimeStandard.getCoordinatedUniversalTime()),
        new JulianDate(2458686.91326173, TimeStandard.getCoordinatedUniversalTime()),
        new Cartesian(4.9782746773377337e10, 2.1886396125292808e11, 5.0570942661222927e10),
        new Cartesian(-19.6851468985649450e3, 7.9694302041760183e3, 7.0701308566945276e3),
        positionEps,
  ...
  public void testHitsEarth() throws Exception {
    ...
    JulianDate epoch =
        new JulianDate(
            ZonedDateTime.parse("2000-01-01T11:59:27.816Z"), TimeStandard.getInternationalAtomicTime());
    JulianDate endTime =
        new JulianDate(
            ZonedDateTime.parse("2020-01-01T11:59:27.816Z"), TimeStandard.getInternationalAtomicTime());
  ...
  public void testOnlySun() throws Exception {
    ...
    Assert.assertTrue("deltaPosition is too large " + deltaPosition.getMagnitude(),
        deltaPosition.getMagnitude() < positionEps);
    Assert.assertTrue("deltaVelocity is too large " + deltaVelocity.getMagnitude(),
        deltaVelocity.getMagnitude() < velocityEps);
    for (int i = 0; i < 3; i++) {
      Assert.assertEquals("expected position component " + i, expectedPosition.get(i), position.get(i), positionEps);
      Assert.assertEquals("expected velocity component " + i, expectedVelocity.get(i), velocity.get(i), velocityEps);
    }
  }
  ...
  public void testForwardsAndBackwards() throws Exception {
    ...
    OrbitParameterMessage forwardsOpm = opmTemplate.deepCopy()
        .setState_vector(
            new StateVector()
                .setEpoch(initialTime.toDateTime(TimeStandard.getCoordinatedUniversalTime()).toString())
    ...
    OrbitParameterMessage backwardsOpm = opmTemplate.deepCopy()
        .setState_vector(
            new StateVector()
                .setEpoch(finalTime.toDateTime(TimeStandard.getCoordinatedUniversalTime()).toString())
    ...
    
    {
      PropagatedInterplanetaryOrbit orbitForwards =
          PropagatedInterplanetaryOrbit.fromOpm(
              forwardsOpm, finalTime, SOLAR_SYSTEM_NO_GR_CONFIG);
  ...
  public void testAsteroidsSmoke() throws Exception {
    ...
    config.addAsteroid("iris").addAsteroid("Hygeia").addAsteroid("Eunomia").addAsteroid("Psyche")
        .addAsteroid("Amphitrite").addAsteroid("Europa").addAsteroid("Cybele").addAsteroid("Sylvia")
        .addAsteroid("Thisbe");

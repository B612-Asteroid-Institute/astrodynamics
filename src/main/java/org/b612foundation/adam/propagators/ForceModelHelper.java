...
/**
 * Helper methods for setting up forces of the solar system correctly.
 */
public class ForceModelHelper {
  ...
  protected static final JplDE JPL_DE = new JplDE430(new ClasspathStreamFactory(DE_FILE));
  ...
  public static RungeKuttaFehlberg78Integrator getRungeKuttaFehlberg78Integrator() {
    // TODO: We probably want to pull all the constants out into solver config that would be specifiable by the user.
   ...
   */
  public static void initializeEarthCenteredForces(PropagatorConfiguration config, PropagationNewtonianPoint object) {
  ...
  private static void addThirdBody(String name, PlanetGravityMode mode, JplDECentralBody body,
    ...
    case OMIT:
      ...
      break;
    case POINT_MASS:
      bodies.addThirdBody(name, JPL_DE.getCenterOfMassPoint(body), JPL_DE.getGravitationalParameter(body));
      break;
    default:
      throw new IllegalArgumentException("Do not support " + mode + " for " + name + " in planetary mode");
    }
  }

  /**
   * Sets up sun-centered forces, including other bodies according to the given config. Adds those forces to the given
   * object.
   */
  public static void initializeSunCenteredForces(PropagatorConfiguration config,
      PropagationNewtonianPoint object) {
    // Add Sun's gravity.
    SunCentralBody sun = CentralBodiesFacet.getFromContext().getSun();
    Preconditions.checkArgument(config.getSun() == PlanetGravityMode.POINT_MASS,
        "Expect Sun to be treated as point-mass in the Sun-centered frame, got %s", config.getSun());
    TwoBodyGravity sunGravity = new TwoBodyGravity(object.getIntegrationPoint(), sun,
  ...
  private static void addEarthPointMassGravity(PropagationNewtonianPoint object) {
    ...
    TwoBodyGravity gravity = new TwoBodyGravity(object.getIntegrationPoint(), earth,
  ...
  private static void addEgm96Gravity(PropagationNewtonianPoint object) {
    ...
    InputStream stream = new ForceModelHelper().getClass().getResourceAsStream(EGM96_FILE);
    if (stream == null) {
      throw new RuntimeException("Cannot load gravity model file " + EGM96_FILE);
    }
    SphericalHarmonicGravityModel model = SphericalHarmonicGravityModel
        .readFrom(new BufferedReader(new InputStreamReader(stream)));
    gravity.setGravityField(
        new SphericalHarmonicGravityField(model, degree, order, includeTwoBodyForces, SphericalHarmonicsTideType.NONE));
    gravity.setTargetPoint(object.getIntegrationPoint());
    object.getAppliedForces().add(gravity);
  }
}

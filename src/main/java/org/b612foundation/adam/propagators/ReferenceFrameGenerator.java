...
public class ReferenceFrameGenerator {
  ...
  public ReferenceFrame getReferenceFrame(OdmCommonMetadata.ReferenceFrame referenceFrameName,
    ...
    case TOD:
    default:
      throw new IllegalArgumentException("Reference frame " + referenceFrameName.name() + " not supported.");
    }
  ...
  private Axes getICRFReferenceFrameAxes() {
    return CentralBodiesFacet.getFromContext().getEarth().getInternationalCelestialReferenceFrame().getAxes();
  }

  private Axes getEarthMEME2000ReferenceFrameAxes() {
    // Get the axes from the Earth's MeanEclipticMeanEquinox frame.
    ReferenceFrame earthMeanEclipticMeanEquinox = CentralBodiesFacet.getFromContext().getEarth()
    ...
    AxesFixedAtJulianDate axesEarthMeanEclipticMeanEquinoxJ2000 = new AxesFixedAtJulianDate(axes,
        CentralBodiesFacet.getFromContext().getSun().getInertialFrame().getAxes(), epoch);

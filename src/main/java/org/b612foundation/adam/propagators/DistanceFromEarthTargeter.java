...
public class DistanceFromEarthTargeter {
  ...
  public DistanceFromEarthTargeter() {
    StkLicense.Activate();
    ForceModelHelper.loadStandardObjects();
  }

  /** Mainly for testing, as I expect that we would have a decent idea of how many iterations is reasonable. */
  ...
  /**
   * Configures but does not run a targeting sequence that will compute the initial maneuver required to propagate an
   * object with the given initial state/epoch to a location that is the given distance from the center of the earth
   * (within the given tolerance) when the object reaches its first perigee after the given duration past the initial
   * epoch.
   *
   * @param initialState
   ...
   * @param initialManeuver
   *          Initial maneuver to use. Assumed to be in object velocity orbit normal reference frame. (velocity = x, y =
   *          orbit normal, z = x cross y).
   * @param durationToNearEarth
   *          A slight underestimate of the time it will take to get to the target perigee. The really important thing
   *          is to set this to a duration that will get the object past the previous perigee but not yet to the target
   *          perigee. Beyond that, the closer the better for performance reasons (and for avoiding unexpectedly
   *          detected perigees). 30 days before perigee is a decent rule of thumb.
   * @param targetingParams
   *          Configuration for the targeting, including: 
   *          [targetDistanceFromEarth, km]: Distance from earth to be targeted. 
   *          [initialTargetDistanceFromEarth, km]: Distance from earth to be targeted during the 1st stage of the targeter.
   *          [tolerance, km] Allowed tolerance on the target distance. 
   *          [runNominalOnly]: Whether to run the nominal sequence only (alternative: run targeted sequence). 
   * @param propagatorConfig
   *          Force model configuration for the targeter.
   * @throws AdamRunnableException 
   * 
   */
  // Can we avoid making the user specify the duration? I don't see how, but it's a little janky.
  public void initialize(Motion1<Cartesian> initialState, ReferenceFrame referenceFrame, JulianDate epoch,
      Cartesian initialManeuver, Duration durationToNearEarth, TargetingParameters targetingParams,
      PropagatorConfiguration propagatorConfig) throws AdamRunnableException {
    this.referenceFrame = referenceFrame;

    SunCentralBody sun = CentralBodiesFacet.getFromContext().getSun();
    EarthCentralBody earth = CentralBodiesFacet.getFromContext().getEarth();

    double targetDistanceFromEarth = targetingParams.getTargetDistanceFromEarth() * 1000.0; // km to m
    ...
    double initialTargetDistanceFromEarth = targetingParams.getInitialTargetDistanceFromEarth() * 1000.0; // km to m
    if (initialTargetDistanceFromEarth <= 0){
      initialTargetDistanceFromEarth = 5.0 * targetDistanceFromEarth;
      log.warning("Initial Target Distance from Earth is <= 0; Setting to 5 x targetDistanceFromEarth = " +
      String.format("%f",initialTargetDistanceFromEarth) + " meters");
    ...
    PropagationNewtonianPoint propagationPoint = new PropagationNewtonianPoint(motionId, referenceFrame,
        initialState.getValue(), initialState.getFirstDerivative());
    // STK barfs if no mass is specified, but it's not actually used in the computations so 0 is a fine dummy value.
    propagationPoint.setMass(Scalar.toScalar(0));
    NumericalPropagatorDefinition propagator = getPropagator(propagatorConfig, propagationPoint, epoch);
    ...
    // Maneuver segment. Uses axes relative to the object's velocity vector. X maneuvers are along the velocity vector.
    ...
    ImpulsiveManeuverInformation applyDeltaVDetails = new ImpulsiveManeuverInformation(motionId, initialManeuver);
    applyDeltaVDetails.setOrientation(new AxesVelocityOrbitNormal(applyDeltaVDetails.getPropagationPoint(), sun));
    ...
    DurationStoppingCondition durationStoppingCondition = new DurationStoppingCondition(durationToNearEarth);
    ...
    // It is necessary to set maximum duration because there is a default maximum duration that is generally much too
    // short for our purposes. Null pointer exceptions will result if this is not set to a duration longer than the
    ...
    // @formatter:off
    DelegateStoppingCondition perigeeStoppingCondition = new DelegateStoppingCondition(
        ConditionCheckCallback.of(currentState -> {
          ...
          Motion1<Cartesian> earthPosAndVel = earthEvaluator.evaluate(currentState.getCurrentDate(), 2);
    ...
    // We also need an altitude stopping condition because for some reason the b-plane targeter completely chokes
    // (returns NaN) if run inside the earth.
    ScalarSphericalElement distanceFromEarth = new ScalarSphericalElement(propagationPoint.getIntegrationPoint(),
        earth.getInternationalCelestialReferenceFrame(), SphericalElement.MAGNITUDE);
    ScalarStoppingCondition distanceFromEarthStoppingCondition = new ScalarStoppingCondition(distanceFromEarth,
        1e6 /* 1000 km threshold */ + WorldGeodeticSystem1984.SemimajorAxis, 1e3 /* 1 km tolerance */,
    ...
    TargetedSegmentListDifferentialCorrector bPlaneDifferentialCorrector = new TargetedSegmentListDifferentialCorrector();
    ...
    // Constraint to target the initial target miss distance from Earth's center using the b-plane to guide us.
    double earthG = ForceModelHelper.JPL_DE.getGravitationalParameter(JplDECentralBody.EARTH);
    DelegateBasedConstraint bMagConstraint = new DelegateBasedConstraint(
        DelegateBasedConstraintCallback.of(segmentResults -> {
          DateMotionCollection1<Cartesian> data = segmentResults.getDateMotionCollectionOfOverallTrajectory(motionId,
              targetingFrame);
          int order = 6;
          Point point = new PointInterpolator(targetingFrame, InterpolationAlgorithmType.HERMITE, order, data);

          Scalar bDotTScalar1 = new VectorBPlane(point, earth, earthG, targetingFrame).getBDotT();
          Scalar bDotRScalar1 = new VectorBPlane(point, earth, earthG, targetingFrame).getBDotR();
          ...
          return bMag;
        }), propagateToPerigeeSegment, initialTargetDistanceFromEarth, tolerance);

    bPlaneDifferentialCorrector.getVariables().add(getManeuverXVariable(maneuverSegment, 10.0, .0001));
    bPlaneDifferentialCorrector.getConstraints().add(bMagConstraint);

    // Then, target the actual distance from earth.
    TargetedSegmentListDifferentialCorrector rMagDifferentialCorrector = new TargetedSegmentListDifferentialCorrector();
    ...
    DelegateBasedConstraint rMagConstraint1 = new DelegateBasedConstraint(
        DelegateBasedConstraintCallback.of(segmentResults -> {
          DateMotionCollection1<Cartesian> data = segmentResults.getDateMotionCollectionOfOverallTrajectory(motionId,
              targetingFrame);
          Cartesian lastValue = last(data.getValues());

          // The center of an earth-centered reference frame is the center of the earth, so the magnitude of a
          // coordinate in this frame is equivalent to the distance from the center of the earth.
          return lastValue.getMagnitude();
        }), propagateToPerigeeSegment, targetDistanceFromEarth, tolerance);
    ...
    // Finally, add all of these to a targeting sequence that can be used to actually carry out the computation.
    ...
    if (runNominalOnly) {
      ...
      targetedSegmentList.setOperatorAction(TargetedSegmentListOperatorBehavior.RUN_ACTIVE_OPERATORS);
    }
  ...
  /**
   * Runs the targeter nominal sequence to determine whether the object already falls outside the target distance from
   * earth.
   * 
   * Note: Calling getEphemeris after this without any other intermediate method calls on this object will return the
   * nominal ephemeris.
   */
  public boolean runNominalPropagationToCheckRequiresManeuver(TargetingParameters targetingParams)
    ...
    if (segmentResults instanceof TargetedSegmentListResults) {
      // Compute the distance from earth. If it is already farther than the innermost boundary of tolerable distance, no
      // maneuver is required.
      TargetedSegmentListResults targetedSegmentListResults = (TargetedSegmentListResults) segmentResults;
      ITimeBasedState currentState = targetedSegmentListResults.getFinalPropagatedState();
      Cartesian pos = (Cartesian) currentState.getMotion(motionId).getValue();

      PointEvaluator earthEvaluator = GeometryTransformer
          .observePoint(CentralBodiesFacet.getFromContext().getEarth().getCenterOfMassPoint(), referenceFrame);
    ...
    throw new AdamRunnableException("Unexpected result type while checking whether maneuver is required", null);
  }
  ...
  /**
   * Retrieves the ephemeris from the last run. If this is called after runNominalPropagationToCheckRequiresManeuver,
   * returns the nominal ephemeris. If called after propagate, gets whichever ephemeris the targeter was initialized to
   * produce.
   */
  ...
  /**
   * Retrieves the computed maneuver from the targeter's results. Returns null and logs a warning if targeting was not
   * successful.
   * 
   * Note that this will compute a maneuver to move the object within the given tolerance of the given distance from
   * earth *even* if it is nominally outside that distance. To find out whether the nominal propagation lands the object
   * within the target distance from earth, use requiresManeuver.
   * 
  ...
  public double[] getManeuver() throws AdamRunnableException {
    ...
    if (segmentResults instanceof TargetedSegmentListResults) {
      ...
      for (TargetedSegmentListOperator operator : targetedSegmentList.getOperators()) {
        TargetedSegmentListOperatorResults operatorResults = targetedSegmentListResults.getOperatorResult(operator);
        ...
        if (operatorResults instanceof TargetedSegmentListDifferentialCorrectorResults) {
          TargetedSegmentListDifferentialCorrectorResults dcResults = (TargetedSegmentListDifferentialCorrectorResults) operatorResults;

          MultivariableFunctionSolverResults solverResults = dcResults.getFunctionSolverResults();
          logStepsTaken(solverResults);
          if (dcResults.getCompletedSuccessfully() && dcResults.getConverged() && solverResults.getConverged()) {
            log.info("Operator " + operatorResults.getIdentifier().getName() + " converged after "
                + solverResults.getIterationResults().size() + " iterations.");
            SolvableMultivariableFunctionResults finalResults = solverResults.getFinalIteration().getFunctionResult();
            double[] vars = finalResults.getVariablesUsed();
            for (int i = 0; i < vars.length; i++) {
              totalManeuver[i] += vars[i];
            }
          } else {
            log.warning("Operator " + operatorResults.getIdentifier().getName() + " did not converge.");
            throw new AdamRunnableException(
                "Targeter did not converge after " + solverResults.getIterationResults().size() + " iterations.", null);
          }
        } else {
          log.warning("Operator " + operatorResults.getIdentifier().getName()
  ...
  private StkEphemerisFile getEphemerisFromResults(SegmentResults results, String motionId) {
    DateMotionCollection1<Cartesian> data = results.getDateMotionCollectionOfOverallTrajectory(motionId,
        referenceFrame);

    StkEphemerisFile.EphemerisTimePosVel ephemeris = new StkEphemerisFile.EphemerisTimePosVel();
    ephemeris.setInterpolator(new TranslationalMotionInterpolator(InterpolationAlgorithmType.HERMITE, 6, data));
  ...
  /**
   * Returns a sun-centered propagator for the given point with a force model as defined in the given config.
   */
  private NumericalPropagatorDefinition getPropagator(PropagatorConfiguration config,
  ...
  /**
   * Adds variable to the differential corrector representing modifications to the x component of the delta-v imparted
   * in the given impulsive maneuver segment.
   * 
   * Notes about max step and perturbation:
   * 
   * Max step should be more than the total amount you think you might be wrong, since this is an upper bound on the
   * total maneuver.
   * 
   * Perturbation (the amount that the velocity will be perturbed to calculate the slope of the effect of delta-V for
   * each iteration) should be quite small, since the final maneuvers will be very small and it's important that this be
   * smaller.
   * 
   * TODO: consider figuring out how to make perturbation dynamic. For large steps, a larger perturbation makes sense.
   * But once we get very close to the target value, a perturbation can become too large (relatively), causing us to
   * overshoot the target and endlessly oscillate. Ideally the perturbation would always be some small but significant
   * fraction of the previous step.
   */
  private SegmentPropagatorVariable getManeuverXVariable(ImpulsiveManeuverSegment maneuverSegment, double maxStep,
      double perturbation) {
    ImpulsiveManeuverInformation maneuverSegmentDetails = maneuverSegment.getManeuvers().get(0);
    SegmentPropagatorVariable maneuverVariableX = maneuverSegment.createVariable(maxStep, perturbation,
  ...
  private void logStepsTaken(MultivariableFunctionSolverResults solverResults) {
    ...
    for (MultivariableFunctionSolverIterationResults result : solverResults.getIterationResults()) {
      ...
      if (vars.length == 1) {
        pFine("Iteration " + result.getIteration() + ", " + vars[0] + ", "
            + result.getFunctionResult().getConstraintValues()[0]);
      } else {
        pFine("Iteration " + result.getIteration() + ", " + vars[0] + ", " + vars[1] + ", " + vars[2] + ", "
            + constraints[0]);
      }
    }
  }

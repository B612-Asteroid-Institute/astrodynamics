...
public class SinglePropagation extends AdamObject {
   ...
   *
   * <p>The header contains information about the reference frame of the data, and a UTC timestamp
   * that can be used to interpret the times of the state vectors in the data.
   *
   * <p>The data section contains cartesian state vector coordinates in meters and meters/second in
   * the metadata reference frame, with times given as seconds offset from the metadata timestamp.
   *
   * <p>For a complete specification, see
   * http://help.agi.com/stk/index.htm#stk/importfiles-02.htm#formats.
   *
   * <p>May be null if not requested that ephemeris be kept.
   */
  private String ephemeris;

  /**
   * Calculated state vector at end time as specified in propagation parameters. Equal to the last
   * line of the ephemeris.
   *
   * <p>Format: time, x, y, z, x velocity, y velocity, z velocity.
   *
   * <p>Units: seconds-since-file-epoch, meters, meters, meters, meters/second, meters/second,
   * meters/second
   */
  ...
  public boolean equals(Object obj) {
    ...
    return super.equals(other)
        && Objects.equals(propagationParameters, other.propagationParameters)
        && Objects.equals(ephemeris, other.ephemeris)
        && Objects.equals(finalStateVector, other.finalStateVector);
  }
}

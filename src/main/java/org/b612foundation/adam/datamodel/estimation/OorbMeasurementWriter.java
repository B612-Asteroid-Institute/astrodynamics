...
public final class OorbMeasurementWriter {
  ...
  public OorbMeasurementWriter() {
    this(
        DEFAULT_RA_ERROR_ESTIMATE_ASEC,
        DEFAULT_DEC_ERROR_ESTIMATE_ASEC,
        DEFAULT_VMAG_ERROR_ESTIMATE);
  }

  public OorbMeasurementWriter(
      double raErrorEstimateAsec, double decErrorEstimateAsec, double vMagEstimateError) {
  ...
  public void writeLsstMeasurementsToDesFile(Path desFile, List<LsstMeasurement> measurements)
  ...
  String convertWriteLsstToDesFormat(LsstMeasurement m) {
    ...
    return String.format(
        "%s %16.10f O %14.10f %14.10f %14.10f X %s%13.10f %14.10f %14.10f  0.1000000E+01 X",
        m.getObjectId(),
        m.getMjd(),
        m.getRaDeg(),
        m.getDecDeg(),

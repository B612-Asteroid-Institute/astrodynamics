...
/**
 * Metadata fields common to all three ODM messages.
 * https://public.ccsds.org/Pubs/502x0b2c1.pdf
 */
public class OdmCommonMetadata implements Serializable {
	/**
	 * The origin of the reference frame. The standard does not restrict the
	 * value, but we will use an enum since we do not plan to propagate in
	 * arbitrary frames anyway.
	 */
	...
	/**
	 * Name of the reference frame in which the state vector and optional
	 * Keplerian element data are given. The standard (appendix A2) defines
	 * multiple values. Appendix A3 defines local frames that can be used only for
	 * maneuvers and covariances.
	 */
	public enum ReferenceFrame {
		/**
		 * The two local frames that can be used only for manuevers or covariances.
		 * Radial, Transverse, Normal. Can also be marked as RSW.
		 */
		RTN,
		/**
		 * A local orbital coordinate frame that has the x-axis along the velocity
		 * vector, W along the orbital angular momentum vector, and N completes the
		 * right handed system.
		 */
		TNW,
	...
	/**
	 * The standard (appendix A1) lists multiple recognized time systems. We start
	 * with what we support and extend this set as needed.
	 */
	...
	/**
	 * Spacecraft name for which the orbit state is provided. There is no
	 * CCSDS-based restriction on the value for this keyword, but it is
	 * recommended to use names from the SPACEWARN Bulletin.
	 */
	private String objectName = null;

	/**
	 * Required Object ID. There is no CCSDS-based restriction on the value for
	 * this keyword, but it is recommended that values be the international
	 * spacecraft designator as published in the SPACEWARN Bulletin. Recommended
	 * values have the format YYYYNNNP{PP}, where: YYYY = Year of launch. NNN =
	 * Three digit serial number of launch in year YYYY (with leading zeros) P{PP}
	 * = At least one capital letter for the identification of the part brought
	 * into space by the launch.
	 */
	private String objectId = null;

	/**
	 * Origin of reference frame, which may be a natural solar system body
	 * (planets, asteroids, comets, and natural satellites), including any planet
	 * barycenter or the solar system barycenter, or another spacecraft. There is
	 * no CCSDS-based restriction on the value for this keyword, but for natural
	 * bodies it is recommended to use names from the NASA/JPL Solar System
	 * Dynamics Group at http://ssd.jpl.nasa.gov
	 */
	private CenterName centerName = null;

	/**
	 * Name of the reference frame in which the state vector and/or the Keplerian
	 * element data are given.
	 */

...
/**
 * Class for translating between text-based ODM formats and classes in this
 * package. The CCSDS ODM standard is here:
...
public final class OdmFormatter {
  ...
  /**
   * Returns the string with mutable fields removed to facilitate search for
   * duplicates.
   */
  public static String removeMutableFields(String originalOpm) {
	...
	/**
	 * Parses OPM message as described in the standard. From all user defined
	 * fields, pulls only adam-specific ones.
	 */
	...
	/**
	 * Parses common header lines. Assumes the input lines all have data. On
	...
	/**
	 * Parses common metadata fields among three ODM types. On success removes the
	...
	/**
	 * Parses the long form of the covariance matrix (each element on a separate
	 * line with a variable name). Removes parsed lines from the list. The long
	 * form is used in OPM and OMM. OEM uses a different (short) format for the
	 * same data.
	 */
	...
	/**
	 * Translates the given center name into an enum value, or throws if the name
	 * is not recognized. The names may contain spaces in them and may be
	 * non-unique.
	 */
	...
	/**
	 * Translates the given reference frame name into an enum value, or throws if
	 * the name is not recognized. The names may contain spaces in them and may be
	 * non-unique.
	 */
	...
	/**
	 * Translates the given time system name into an enum value, or throws if the
	...
	/**
	 * Expects the first line in the list to start with the prefix and extracts
	 * numeric value from that line. The line may contain more text (units) after
	 * the number. Throws if the line is not found or the number doesn't parse.
	 * Removes the parsed line from the list.
	 */
	...
	/**
	 * Expects the first line in the list to start with the given prefix. If
	 * found, extracts the field value and removes the line from the list.
	 * Otherwise throws.
	 */
